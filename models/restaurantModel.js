...
function fetchRestaurantsWithQueries(search, sort_by) {
  ...
  return db.query(queryStr)
    .then((response) => {
      ...
      restaurants.forEach(
        (restaurant) => restaurant.average_rating = Number(restaurant.average_rating)
      );
...
function addRestaurant(newRestaurant) {
  // Could use Object.entries then pg-format
...
function fetchAreaByAreaId(areaId) {
  return db.query(`SELECT * FROM areas WHERE area_id = $1;`, [areaId])
    .then((result) => result.rows[0]);
}

function fetchRestaurantsByAreaId(areaId) {
  return db.query(`SELECT * FROM restaurants WHERE area_id = $1;`, [areaId])
